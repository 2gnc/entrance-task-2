...
* по заданию сверстаны статические страницы: 
  *. **```/public/index.html```** - Вид главной страницы. Соответствует макету "Главная".
  *. **```/public/index-calendar.html```** - Вид главной страницы с открытым календарем. Соответствует макету "Главная календарь" для десктопа с тремя месяцами в календаре.
  *. **```/public/new-event.html```** - Создание новой встречи, автокомплита нет, участники выбраны
  *. **```/public/new-eventmob.html```** - Создание новой встречи, автокомплита нет, участники не выбраны
  *. **```/public/event-states.html```** - Состояния встречи, автокомплит показан, переговорная выбрана. В мобильной
  версии нет сообщения "выберите переговорку", тк она уже выбрана.
  *. **```/public/event-editor.html```**  - Страница редактирования встречи.
  *. **```/public/event-delete.html```** - Демонстрация модального окна "Редактирование встречи (удалить)".
  *. **```/public/event-saved.html```** - Демонстрация модального окна "Встреча создана".
  *. **```/public/index-mobmain.html```** - то же, что и index.html, но другое текущее время, нет тултипа и кнопки создания эвента.
  *. **```/public/index-mobmainclick.html```** - клик на эвент в мобильной версии
